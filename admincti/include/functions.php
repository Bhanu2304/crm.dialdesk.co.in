...
function createDateRangeArray($strDateFrom,$strDateTo)
	...
	function lookup_gmt2($phone_code,$USarea,$state,$LOCAL_GMT_OFF_STD,$Shour,$Smin,$Ssec,$Smon,$Smday,$Syear,$postalgmt,$postal_code,$owner){
		global $link;
		...
		if ( ($postalgmt=="TZCODE") && (strlen($owner)>1) )
			...
			$rslt=mysql_to_mysqli($stmt,$link);
			$pc_recs = mysqli_num_rows($rslt,$link);
			if ($pc_recs > 0)
				{
				$row=mysqli_fetch_row($rslt);
				$dst_range =    $row[0];
		...
		if ($postalgmt_found < 1)
			...
			if ($phone_code =='52')
				{
				$stmt="select country_code,country,areacode,state,GMT_offset,DST,DST_range,geographic_description from asterisk.vicidial_phone_codes where country_code='$phone_code' and areacode='$USarea';";
				$rslt=$rslt=mysql_to_mysqli($stmt,$link);
				$pc_recs = mysqli_num_rows($rslt);
				if ($pc_recs > 0)
					{
					$row=mysqli_fetch_row($rslt);
