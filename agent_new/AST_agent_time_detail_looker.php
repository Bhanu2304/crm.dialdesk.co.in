...
if (isset($_GET["query_tms"]))				{$query_tms=$_GET["query_tms"];}
	elseif (isset($_POST["query_tms"]))		{$query_tms=$_POST["query_tms"];}
//echo $query_tms;
if (isset($_GET["query_tme"]))				{$query_tme=$_GET["query_tme"];}
	elseif (isset($_POST["query_tme"]))		{$query_tme=$_POST["query_tme"];}
//echo $query_tme;
//print_r($_REQUEST);
...
$LINKbase = "$PHP_SELF?query_date=$query_date&end_date=$end_date&query_tms=$query_tms&query_tme=$query_tme$groupQS$user_groupQS&shift=$shift&show_parks=$show_parks&time_in_sec=$time_in_sec&search_archived_data=$search_archived_data&report_display_type=$report_display_type&DB=$DB";
...
#	echo "\n";
	...
	if ($shift == 'ALL') 
		{
		if (strlen($time_BEGIN) < 6) {$time_BEGIN = $query_tms;}
		if (strlen($time_END) < 6) {$time_END = $query_tme;}
		}
	if ($shift == '9AM-5PM') 
		{
		$time_BEGIN = "09:00:00";
	...
	$stmt="select $userSQL,wait_sec,talk_sec,dispo_sec,if(pause_sec>50000,100,pause_sec) pause_sec ,lead_id,status,dead_sec,campaign_id from ".$agent_log_table." where event_time <= '$query_date_END' and event_time >= '$query_date_BEGIN' $group_SQL $user_group_SQL limit 10000000;";
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {$ASCII_text.= "$stmt\n";}
	...
	while ($i < $rows_to_print)
		...
		if ($customer < 1)
			{$customer=0;}
		//if ($campaign_id!='CHAT2O' && $lead=='') {$wait=0;}
	...
	while ( ($m < $uc) and ($m < 50000) )
		...
		$RAWcalls = $Scalls[$m];
		$RAWtimeSEC = $Stime[$m];
		$ACHt=round(($Stalk[$m]+$Sdispo[$m])/$Scalls[$m]);
...
$Login = mysqli_fetch_row(mysql_to_mysqli("select time(min(event_date)) logintime from asterisk.vicidial_user_log where user='$Suser[$m]' and event='Login' and event_date >= '$query_date_BEGIN' and event_date <= '$query_date_END'", $link));

$LoginT = $Login[0]==''?"00:00:00":"$Login[0]";

$Logout = mysqli_fetch_row(mysql_to_mysqli("select time(max(event_date)) logouttime from asterisk.vicidial_user_log where user='$Suser[$m]' and event='LOGOUT' and event_date >= '$query_date_BEGIN' and event_date <= '$query_date_END'", $link));

$LogoutT = $Logout[0]==''?"00:00:00":"$Logout[0]";
		...
		if ($file_download < 1)
			...
			if ($atdr_login_logout_user_link > 0)
				...
				{
				$Toutput = "| $Sname[$m] | <a href=\"./user_stats.php?user=$RAWuser&begin_date=$query_date&end_date\">$Suser[$m]</a> | $Scalls[$m] | $StimeTC[$m]$TCuserAUTOLOGOUT| $Stime[$m] |$park_AGENT_INFO $Swait[$m] | $Swaitpct[$m] | $Stalk[$m] | $Stalkpct[$m] | $Sdispo[$m] | $Sdispopct[$m] | $Spause[$m] | $Spausepct[$m] | $Sdead[$m] | $Sdeadpct[$m] | $Scustomer[$m] | $LoginT   | $LogoutT   |    $ACHt	  |   |$SstatusesHTML\n";
				}
			...
			$fileToutput = "$RAWname,$RAWuser,$RAWcalls,$RAWtimeTC,$RAWtime,$park_AGENT_INFO_CSV$RAWwait,$RAWwaitpct %,$RAWtalk,$RAWtalkpct %,$RAWdispo,$RAWdispopct %,$RAWpause,$RAWpausepct %,$RAWdead,$RAWdeadpct %,$RAWcustomer,$LoginT,$LogoutT,$ACHt$SstatusesFILE\n";
			}
...
echo "<SELECT NAME=query_tme>";
	 
	echo "<option value=23:59:59>00:00:00</option>";
	echo "<option value=01:00:00>01:00:00</option>";
	echo "<option value=02:00:00>02:00:00</option>";
echo "<option value=03:00:00>03:00:00</option>";
echo "<option value=04:00:00>04:00:00</option>";
