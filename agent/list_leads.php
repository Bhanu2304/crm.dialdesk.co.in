...
if(isset($_POST['SUBMIT']))
{
  ...
  #$Whrqry = " and user='".$_SESSION['SESS_USER']."'"; // Start with a true condition
  ...
  if($status !== "All") {
      $Whrqry .= " and  status='$status'";
  }
  // if($subStatus !== "All") {
  //     $Whrqry .= " and user='".$_SESSION['SESS_USER']."'";
  // }
...
$selectQuery = "SELECT * FROM vicidial_list where list_id='$list_id' $Whrqry";
                          ...
                          <div class="row">
                            <div class='col-sm-3'>
                              ...
                              <?php
                                        ...
                                        $qry_country_list = "SELECT list_id FROM vicidial_list GROUP BY list_id";
                                        ...
                                        while($cntr=mysqli_fetch_assoc($rsc_country_list)){
                                            $selected = ($cntr['list_id'] == $list_id) ? "selected" : "";
                                            echo "<option value=\"{$cntr['list_id']}\" $selected>{$cntr['list_id']}</option>";
                                        }
                                        echo "</select>";
                                        ?>
                            </div>
                            <div class='col-sm-3'>
                              ...
                              <?php
                                  ...
                                  $qry_country_list = "SELECT state FROM vicidial_list WHERE state!='' GROUP BY state";
                                  $rsc_country_list = mysqli_query($link,$qry_country_list);
                                  $value_list_str = [];
                            ...
                            <div class='col-sm-3'>
                              ...
                              <select name="status" class="form-control">
                                  ...
                                  <option value="CBHOLD">CBHOLD - Scheduled Callback</option>
                              </select>
                            </div>
                          ...
                          <tr>
                            ...
                            <tbody>
                                ...
                                { #print_r($row);
                                    ...
                                    echo "<tr id='row_{$row['lead_id']}'>";
                                    echo "<td><a href=\"#\" onClick=\"fetchAndDisplayResults({$row['phone_number']})\" />{$row['phone_number']} </a><input type=\"hidden\" name=\"output\" id=\"output\" value=\"{$row['phone_number']}\"></td>";
                                    echo "<td>{$row['lead_id']}</td>";
                                    echo "<td>{$row['first_name']} {$row['last_name']}</td>";
                                    echo "<td>{$row['email']}</td>";
                                    echo "<td>{$row['city']}</td>";
                                    echo "<td>{$row['state']}</td>";
                                    echo "<td>{$row['last_local_call_time']}</td>";
                                    echo "<td>{$row['status']}</td>";
                                    echo "<td><a href='#' onclick=\"showPopup('{$row['lead_id']}')\" data-toggle='modal' data-target='#myModal'><i class='mdi mdi-cloud-download'></i></a></td>";
