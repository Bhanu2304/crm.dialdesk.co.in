...
$phone_login=$_SESSION['phone_user'];
$phone_pass=$_SESSION['phone_pass'];
$VD_login=$_SESSION['user'];;
$VD_pass=$_SESSION['pass'];;
...
$stmt_system_det = "SELECT use_non_latin,vdc_header_date_format,vdc_customer_date_format,vdc_header_phone_format,webroot_writable,timeclock_end_of_day,vtiger_url,enable_vtiger_integration,outbound_autodial_active,enable_second_webform,user_territories_active,static_agent_url,custom_fields_enabled,pllb_grouping_limit,qc_features_active,allow_emails,callback_time_24hour,enable_languages,language_method,meetme_enter_login_filename,meetme_enter_leave3way_filename,enable_third_webform,default_language,active_modules,allow_chats,chat_url,default_phone_code,agent_screen_colors,manual_auto_next,agent_xfer_park_3way,admin_web_directory,agent_script,agent_push_events,agent_push_url,agent_logout_link,agentonly_callback_campaign_lock,manual_dial_validation,mute_recordings FROM system_settings;";
$rslt_system=mysql_to_mysqli($stmt_system_det, $link);
	if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt_system_det,'01001',$VD_login,$server_ip,$session_name,$one_mysql_log);}
if ($DB) {echo "$stmt_system_det\n";}
$qm_conf_ct = mysqli_num_rows($rslt_system);
if ($qm_conf_ct > 0)
	{
            $system_det=mysqli_fetch_assoc($rslt);
            //print_r($system_det);exit;
            $non_latin =						$system_det['use_non_latin'];
            $vdc_header_date_format =			$system_det['vdc_header_date_format'];
            $vdc_customer_date_format =			$system_det['vdc_customer_date_format'];
            $vdc_header_phone_format =			$system_det['vdc_header_phone_format'];
            $WeBRooTWritablE =					$system_det['webroot_writable'];
            $timeclock_end_of_day =				$system_det['timeclock_end_of_day'];
            $vtiger_url =						$system_det['vtiger_url'];
            $enable_vtiger_integration =		$system_det['enable_vtiger_integration'];
            $outbound_autodial_active =			$system_det['outbound_autodial_active'];
            $enable_second_webform =			$system_det['enable_second_webform'];
            $user_territories_active =			$system_det['user_territories_active'];
            $static_agent_url =					$system_det['static_agent_url'];
            $custom_fields_enabled =			$system_det['custom_fields_enabled'];
            $SSpllb_grouping_limit =			$system_det['pllb_grouping_limit'];
            $qc_enabled =						$system_det['qc_features_active'];
            $email_enabled =					$system_det['allow_emails'];
            $callback_time_24hour =				$system_det['callback_time_24hour'];
            $SSenable_languages =				$system_det['enable_languages'];
            $SSlanguage_method =				$system_det['language_method'];
            $meetme_enter_login_filename =		$system_det['meetme_enter_login_filename'];
            $meetme_enter_leave3way_filename =	$system_det['meetme_enter_leave3way_filename'];
            $enable_third_webform =				$system_det['enable_third_webform'];
            $default_language =					$system_det['default_language'];
            $active_modules =					$system_det['active_modules'];
            $chat_enabled =						$system_det['allow_chats'];
            $_SESSION['chat_enabled'] = $chat_enabled;
            $chat_URL =							$system_det['chat_url'];
            $default_phone_code =				$system_det['default_phone_code'];
            $agent_screen_colors =				$system_det['agent_screen_colors'];
            $SSmanual_auto_next =				$system_det['manual_auto_next'];
            $SSagent_xfer_park_3way =			$system_det['agent_xfer_park_3way'];
            $admin_web_directory =				$system_det['admin_web_directory'];
            $SSagent_script =					$system_det['agent_script'];
            $agent_push_events =				$system_det['agent_push_events'];
            $agent_push_url =					$system_det['agent_push_url'];
            $_SESSION["agent_push_url"] = $agent_push_url;
            $agent_logout_link =				$system_det['agent_logout_link'];
            $agentonly_callback_campaign_lock = $system_det['agentonly_callback_campaign_lock'];
            $SSmanual_dial_validation =			$system_det['manual_dial_validation'];
            $SSmute_recordings =				$system_det['mute_recordings'];
	}
...
$_SESSION['LogouTKicKAlL'] = $LogouTKicKAlL;
	...
	if(1)
		...
		if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
			...
			$VICIDiaL_park_on_extension = "$park_ext";
                        echo $VICIDiaL_park_on_extension;exit;
...
if ($callback_display_days > 0)
	{$cb_display_days=$callback_display_days;}
        ...
        $sess_dd= "phone_login={$_SESSION['phone_user']};phone_pass={$_SESSION['phone_pass']};VD_login={$_SESSION['user']};VD_pass={$_SESSION['pass']};VD_campaign = {$_SESSION['VD_campaign']};";
//echo $sess_dd; exit;
