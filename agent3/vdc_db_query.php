...
$SIP_user = $_SESSION['SIP_user'];
$VD_login=$_SESSION['user'];

$campaign = $_SESSION['VD_campaign'];
$server_id_qry="SELECT conf_exten FROM vicidial_conferences where extension='$SIP_user' and server_ip = '$server_ip' LIMIT 1;";
$rsc_server_id=mysqli_query($link, $server_id_qry);
$server_det = mysqli_fetch_assoc($rsc_server_id);
$session_id =$server_det['conf_exten'];


if(empty($server_det))
{
    $find_conference_qry2="SELECT count(*) FROM vicidial_conferences where server_ip='$server_ip' and ((extension='') or (extension is null));";
    $rsc_conference=mysqli_query($link, $find_conference_qry2);
    $conf_det = mysqli_fetch_assoc($rsc_conference);
    if(!empty($conf_det))
    {
        //$upd_conference="UPDATE vicidial_conferences set extension='$SIP_user', leave_3way='0' where server_ip='$server_ip' and ((extension='') or (extension is null)) limit 1;";
        //$rsc_upd_conf=mysqli_query($link, $upd_conference);
       // $conf_det = mysqli_fetch_assoc($rsc_upd_conf);
        
        $server_id_qry2="SELECT conf_exten from vicidial_conferences where server_ip='$server_ip' and ( (extension='$SIP_user') or (extension='$user') );";
        $rsc_server_id2=mysqli_query($link, $server_id_qry2);
        $server_det2 = mysqli_fetch_assoc($rsc_server_id2);
        $session_id =$server_det2['conf_exten'];
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ($show_campaign_list > 0)
		...
		echo "<select size=1 name=VD_campaign id=VD_campaign class=form-control>\n";
		echo "<option value=\"\">-- "._QXZ("Select")." --</option>\n";
	...
	if ( ( (strlen($phone_number)<1) and (strlen($email)<1) ) or (strlen($lead_id)<1) )
		...
		while ( ($errno > 0) and ($retry_count < 9) )
			...
			$retry_count++;
                ...
                //insertion in table call master entrycampaign_id='$campaign'
                ...
                $qry_field_list2 = "SELECT * FROM field_master  where campaign_id='$campaign'";
                ...
                $qry_columns['campaign_id'] = $campaign;
                $qry_columns['created_at'] = date('Y-m-d H:i:s');
                $qry_columns['tag_by'] = $_POST['user'];
                ...
                {
                   $sel_max_tic_no = "select max(ticket_no_auto) max_tno from call_master where campaign_id='$campaign' ;"; 
                ...
                {
                   $sel_max_tic_no = "select max(order_id_auto) max_ordno from call_master where campaign_id='$campaign' ;"; 
                   $rslt_max_tic_no=mysql_to_mysqli($sel_max_tic_no, $link);
                   $last_ticket_det = mysqli_fetch_assoc($rslt_max_tic_no);
